...
BBox * ColorTracker::track(cv::Mat & img, double x1, double y1, double x2, double y2)
{
    ...
    if (std::abs(std::log(scale)) > 0.05){
        ...
        im3 = im3_old;
        double scaleB = 1;
        std::cout << "/* check point output hamesha nahi hoga pr */" << '\n';
        std::cout<<histMeanShiftIsotropicScale(modeCenter.x - width/2, modeCenter.y - height/2, modeCenter.x + width/2, modeCenter.y + height/2, &scaleB)<<std::endl;
        im1 = tmp_im1;
        im2 = tmp_im2;
        im3 = tmp_im3;
...
void ColorTracker::extractForegroundHistogram(int x1, int y1, int x2, int y2, Histogram & hist)
{
    ...
    for (int y = y1; y < y2+1; ++y){
        ...
        for (int x = x1; x < x2+1; ++x){
          ...
          //  if(x==x1 && y==y1) std::cout <<(int)im1.at<uchar>(y,x)<<"  f   "<<(double)M1[x] << '\n';
